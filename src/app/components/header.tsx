...
function Logo({ closeMenu }: { closeMenu: () => void }) {
	return (
		<Link href='/' onClick={closeMenu} className="flex items-center space-x-2">
			<Image src='/logos/sembezaafrica_logo.png' alt='Sembeza Africa logo' priority width={96} height={96} className='w-20 md:w-24' />
		</Link>
	)
}
...
function NavLinks({ pathname, className, onClick }: { pathname: string, className?: string, onClick?: () => void }) {
	return (
		<nav className={clsx(className, "text-green-800")}>
			...
			<Link href="/what-we-do" onClick={onClick} className={clsx(
				pathname === '/what we do' ? 'underline' : 'no-underline',
				'hover:underline transition-all md:text-xl aria-disabled:'
			)}>
				What we do
			</Link>
			<Link href="/ways-to-help" onClick={onClick} className={clsx(
				pathname === '/ways-to-help' ? 'underline' : 'no-underline',
				'hover:underline transition-all md:text-xl'
			)}>
				Ways to help
			</Link>
