  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-start bg-white min-h-screen px-8">
      <div className="flex justify-between items-center mb-8">
        <Logo closeMenu={closeMenu} />
        <Button variant='ghost' onClick={closeMenu} className="text-lg font-semibold text-[#505A28]">
          Close
        </Button>
      </div>
      ...
      {session?.data?.user && (
        <Button variant='ghost' className='self-end mt-4' onClick={closeMenu}>
          <Link href='/account' className='py-2 text-xl text-[#505A28]'>My Account</Link>
        </Button>
      )}
    </div>
  );
